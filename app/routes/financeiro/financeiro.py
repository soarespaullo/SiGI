...
def entradas():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(nova)
        db.session.commit()
        registrar_log(current_user.email, f"Registrou entrada: {nova.descricao}", "sucesso")  # 👈 log
...
def excluir_entrada(id):
    ...
    try:
        ...
        db.session.commit()
        registrar_log(current_user.email, f"Excluiu entrada: {entrada.descricao}", "sucesso")  # 👈 log
        flash("Entrada excluída com sucesso!", "success")
    except Exception:
        db.session.rollback()
        registrar_log(current_user.email, f"Erro ao excluir entrada: {entrada.descricao}", "erro")  # 👈 log
...
def editar_entrada(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        registrar_log(current_user.email, f"Editou entrada: {entrada.descricao}", "sucesso")  # 👈 log
...
def saidas():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(nova)
        db.session.commit()
        registrar_log(current_user.email, f"Registrou saída: {nova.descricao}", "sucesso")  # 👈 log
...
def excluir_saida(id):
    ...
    try:
        ...
        db.session.commit()
        registrar_log(current_user.email, f"Excluiu saída: {saida.descricao}", "sucesso")  # 👈 log
        flash("Saída excluída com sucesso!", "success")
    except Exception:
        db.session.rollback()
        registrar_log(current_user.email, f"Erro ao excluir saída: {saida.descricao}", "erro")  # 👈 log
...
def editar_saida(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        registrar_log(current_user.email, f"Editou saída: {saida.descricao}", "sucesso")  # 👈 log
...
def relatorios():
    ...
    registrar_log(current_user.email, "Gerou relatório financeiro", "sucesso")  # 👈 log
    return render_template(
...
def export_csv():
    ...
    registrar_log(current_user.email, "Exportou relatório financeiro em CSV", "sucesso")  # 👈 log
...
def comprovantes():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(novo)
        db.session.commit()
        registrar_log(current_user.email, f"Enviou comprovante: {filename}", "sucesso")  # 👈 log
